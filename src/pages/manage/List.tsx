...
const List: FC = () => {
  ...
  const [questionList, setQuestionList] = useState([]); // 全部的列表数据，上划加载更多，累计
  const [total, setTotal] = useState(0);
  const haveMoreData = total > questionList.length; // 判断是否有未加载的数据
  ...
  const { run: load, loading } = useRequest(
    ...
    },
    {
      ...
      onSuccess(result) {
        ...
        setQuestionList(questionList.concat(l)); // 累计
        setTotal(total);
        setPage(page + 1);
      },
    }
